...
export const TextbookEdit = (props: EditProps) => {
	...
	const handleUpdate = async (values: any) => {
		...
		try {
			...
			await dataProvider.update('textbooks', { id, data })
			notify('Успешно обновихте учебника', { type: 'success' })
			redirect(`/admin/textbooks/${id}/show`)
		} catch (error) {
			console.error(error)
	...
	return (
		<Edit {...props} title={'Редактиране на учебник'}>
			<Typography
				sx={{
					marginTop: 2,
					textAlign: 'center',
				...
				>
					...
					<Box sx={{ width: '100%' }}>
						...
						>
							...
							/>
							<ReferenceArrayInput
								reference='categories/textbooks'
								source='id'
							...
							>
								<AutocompleteArrayInput
									...
									create={<CreateCategory categoryType={'textbooks'} />}
								/>
							</ReferenceArrayInput>
